...
public class PbTextParser implements PsiParser, LightPsiParser {
  ...
  static boolean BraceMessageEntry(PsiBuilder builder, int level) {
    ...
    exit_section_(builder, level, marker, result, pinned, MessageRecovery_parser_);
    return result || pinned;
  }
  ...
  static boolean RootMessageEntry(PsiBuilder builder, int level) {
    ...
    pinned = result; // pin = 1
    result = result && Field(builder, level + 1);
    exit_section_(builder, level, marker, result, pinned, RootMessageRecovery_parser_);
    return result || pinned;
  }
  ...
  static final Parser RootMessageRecovery_parser_ = new Parser() {
    public boolean parse(PsiBuilder builder, int level) {
      return RootMessageRecovery(builder, level + 1);
    }
  };
}
